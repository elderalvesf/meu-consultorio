...
class PaymentRepository @Inject constructor(
    ...
    fun getPaymentsByPatient(patientId: Long): Flow<List<Payment>> = dao.getPaymentsByPatient(patientId)
    ...
    fun getTotalReceivedInRange(start: Long, end: Long): Flow<Double?> = dao.getTotalReceivedInRange(start, end)
    fun getPaymentsByRange(start: Long, end: Long): Flow<List<Payment>> = dao.getPaymentsByRange(start, end)

    suspend fun insertPayment(payment: Payment): Long {
        val id = dao.insertPayment(payment)
        sync.pushPayment(payment.copy(id = id))
        return id
    }
